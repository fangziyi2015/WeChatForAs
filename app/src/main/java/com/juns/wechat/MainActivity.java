...
//		startService(intent);
	...
	private OnItemOnClickListener onitemClick = new OnItemOnClickListener() {
		...
		public void onItemClick(ActionItem item, int position) {
			...
			switch (position) {
			case 0:// 发起群聊
				Utils.openActivity(MainActivity.this,
						AddGroupChatActivity.class);
				break;
			case 1:// 添加朋友
				Utils.openActivity(MainActivity.this, PublicActivity.class,
						new BasicNameValuePair(Constants.NAME, "添加朋友"));
				break;
			case 2:// 扫一扫
				Utils.openActivity(MainActivity.this, CaptureActivity.class);
				break;
			case 3:// 收钱
				Utils.openActivity(MainActivity.this, GetMoneyActivity.class);
				break;
			default:
				break;
			}
...
//		IntentFilter intentFilter = new IntentFilter(EMClient.getInstance().chatManager()
		...
		public void onDisconnected(final int error) {
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					} else if (error == EMError.CALL_CONNECTION_ERROR) {
						// 显示帐号在其他设备登陆dialog
						// showConflictDialog();
					} else {
	...
	private class MyGroupChangeListener implements EMGroupChangeListener {
		...
		public void onInvitationReceived(String groupId, String groupName,
			...
			if (user != null) {
				...
				msg.addBody(new EMTextMessageBody(user.getUserName() + st3));
				msg.setAttribute("useravatar", user.getHeadUrl());
				msg.setAttribute("usernick", user.getUserName());
				// 保存邀请消息
				EMClient.getInstance().chatManager().saveMessage(msg);
				// 提醒新消息
//				EMClient.getInstance().contactManager().notifyOnNewMsg();
			}
			runOnUiThread(new Runnable() {
				public void run() {
					updateUnreadLabel();
...
//			EMNotifier.getInstance(getApplicationContext()).notifyOnNewMsg();
	...
	public void updateUnreadLabel() {
		int count = 0;
		count = EMClient.getInstance().chatManager().getUnreadMsgsCount();
		if (count > 0) {
