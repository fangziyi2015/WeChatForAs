...
public class Fragment_Msg extends Fragment implements OnClickListener,
	...
	private void initViews() {
		...
		if (conversationList != null && conversationList.size() > 0) {
			...
			EMConversation nee = EMClient.getInstance().chatManager().getConversation("100000");
	...
	private List<EMConversation> loadConversationsWithRecentChat() {
		...
		Hashtable<String, EMConversation> conversations =
				(Hashtable<String, EMConversation>) EMClient.getInstance().chatManager()
				.getAllConversations();
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		if (adpter.PublicMsg != null && position == 0) {
			// 打开订阅号列表页面
			Utils.openActivity(getActivity(), PublishMsgListActivity.class);
		} else {
			...
			if (ChatRecord != null) {
				if (conversation.isGroup()) {
					GroupInfo info = GloableParams.GroupInfos.get(conversation.getLastMessage()
							.getUserName());
					if (info != null) {
					...
					User user = GloableParams.Users.get(conversation.getLastMessage()
							.getUserName());
					if (user != null) {
